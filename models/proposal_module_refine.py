...
UPPER_THRESH_1 = 100.0
SURFACE_THRESH_1 = 0.5
LINE_THRESH_1 = 0.5


UPPER_THRESH_2 = 100.0
SURFACE_THRESH_2 = 0.55
LINE_THRESH_2 = 0.55

UPPER_THRESH_3 = 100.0
SURFACE_THRESH_3 = 0.6
LINE_THRESH_3 = 0.6
...
class ProposalModuleRefine1(nn.Module):
    ...
    def forward(self, xyz, features, center_z, z_feature, center_xy, xy_feature, center_line, line_feature, end_points):
        ...
        end_points["pred_z_ind"] = (ind_normal_z[:,1,:] > SURFACE_THRESH_1).detach().float()
        z_sel = (ind_normal_z[:,1,:] <= SURFACE_THRESH_1).detach().float()
        offset = torch.ones_like(center_z) * UPPER_THRESH_1
        ...
        end_points["pred_xy_ind"] = (ind_normal_xy[:,1,:] > SURFACE_THRESH_1).detach().float()
        xy_sel = (ind_normal_xy[:,1,:] <= SURFACE_THRESH_1).detach().float()
        offset = torch.ones_like(center_xy) * UPPER_THRESH_1
        ...
        end_points["pred_line_ind"] = (ind_normal_line[:,1,:] > LINE_THRESH_1).detach().float()
        line_sel = (ind_normal_line[:,1,:] <= SURFACE_THRESH_1).detach().float()
        offset = torch.ones_like(center_line) * UPPER_THRESH_1
...
class ProposalModuleRefine2(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling, seed_feat_dim=256, with_angle=False):
        super().__init__() 
    ...
    def forward(self, xyz, features, center_z, z_feature, center_xy, xy_feature, center_line, line_feature, end_points):
        ...
        ### Extract surface points and features here
        ind_normal_z = self.softmax_normal(end_points["pred_flag_z"])
        end_points["pred_z_ind"] = (ind_normal_z[:,1,:] > SURFACE_THRESH_2).detach().float()
        z_sel = (ind_normal_z[:,1,:] <= SURFACE_THRESH_2).detach().float()
        offset = torch.ones_like(center_z) * UPPER_THRESH_2
        z_center = center_z + offset*z_sel.unsqueeze(-1)
        z_sem = end_points["sem_cls_scores_z"]

        ind_normal_xy = self.softmax_normal(end_points["pred_flag_xy"])
        end_points["pred_xy_ind"] = (ind_normal_xy[:,1,:] > SURFACE_THRESH_2).detach().float()
        xy_sel = (ind_normal_xy[:,1,:] <= SURFACE_THRESH_2).detach().float()
        offset = torch.ones_like(center_xy) * UPPER_THRESH_2
        xy_center = center_xy + offset*xy_sel.unsqueeze(-1)
        xy_sem = end_points["sem_cls_scores_xy"]
        ...
        ### Extract line points and features here
        ind_normal_line = self.softmax_normal(end_points["pred_flag_line"])
        end_points["pred_line_ind"] = (ind_normal_line[:,1,:] > LINE_THRESH_2).detach().float()
        line_sel = (ind_normal_line[:,1,:] <= SURFACE_THRESH_2).detach().float()
        offset = torch.ones_like(center_line) * UPPER_THRESH_2
        line_center = center_line + offset*line_sel.unsqueeze(-1)
        end_points['line_center_pred'] = line_center
        end_points['line_sem_pred'] = end_points["sem_cls_scores_line"]
...
class ProposalModuleRefine3(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling, seed_feat_dim=256, with_angle=False):
        super().__init__() 
    ...
    def forward(self, xyz, features, center_z, z_feature, center_xy, xy_feature, center_line, line_feature, end_points):
        ...
        ### Extract surface points and features here
        ind_normal_z = self.softmax_normal(end_points["pred_flag_z"])
        end_points["pred_z_ind"] = (ind_normal_z[:,1,:] > SURFACE_THRESH_3).detach().float()
        z_sel = (ind_normal_z[:,1,:] <= SURFACE_THRESH_3).detach().float()
        offset = torch.ones_like(center_z) * UPPER_THRESH_3
        z_center = center_z + offset*z_sel.unsqueeze(-1)
        z_sem = end_points["sem_cls_scores_z"]

        ind_normal_xy = self.softmax_normal(end_points["pred_flag_xy"])
        end_points["pred_xy_ind"] = (ind_normal_xy[:,1,:] > SURFACE_THRESH_3).detach().float()
        xy_sel = (ind_normal_xy[:,1,:] <= SURFACE_THRESH_3).detach().float()
        offset = torch.ones_like(center_xy) * UPPER_THRESH_3
        xy_center = center_xy + offset*xy_sel.unsqueeze(-1)
        xy_sem = end_points["sem_cls_scores_xy"]
        ...
        ### Extract line points and features here
        ind_normal_line = self.softmax_normal(end_points["pred_flag_line"])
        end_points["pred_line_ind"] = (ind_normal_line[:,1,:] > LINE_THRESH_3).detach().float()
        line_sel = (ind_normal_line[:,1,:] <= SURFACE_THRESH_3).detach().float()
        offset = torch.ones_like(center_line) * UPPER_THRESH_3
        line_center = center_line + offset*line_sel.unsqueeze(-1)
        end_points['line_center_pred'] = line_center
        end_points['line_sem_pred'] = end_points["sem_cls_scores_line"]
