...
class PrimitiveModule(nn.Module):
    ...
    def forward(self, xyz, features, end_points, mode=''):
        ...
        features = features.contiguous().view(batch_size, feature_dim, 32, 32)
        net = self.sa1(features)
        net = self.sa2(net)
        net = net.contiguous().view(batch_size, feature_dim, 1024)
        #net = net.contiguous().view(batch_size, feature_dim, self.num_proposal)
        features = features.contiguous().view(batch_size, feature_dim, 1024)
        ...
        global_features = torch.cat((global_features.expand(features.shape[0], 256+128, 1024), net),1)
